...
# These tokens are useful for Encoding and Decoding in the Seq2Seq model
...
# Adding the EOS (End Of String) token to the end of every answer
...
# this will speed up the training (by removing the amount of padding) 
# and will help to reduce the loss
...
# Creating tensorflow placeholders for the inputs and the targets
def model_inputs():
...
# Preprocessing the targets (because the decoder will only accept a certain format of the targets)
# the targets must be in batches for feeding into the decoder
# each of the answers in the batch of targets must start with the SOS tokens
